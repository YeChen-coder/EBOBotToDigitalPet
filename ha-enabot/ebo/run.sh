...
# Standalone Docker supplies the stable network alias "ebo-engine". Honour it instead of
# replacing it with Docker's ephemeral container id; HA/go2rtc caches a camera source URL, so a
# container-id hostname makes the native camera card fail after every recreate.
export EBO_API_HOST="${EBO_API_HOST:-$(hostname 2>/dev/null || hostname -i 2>/dev/null || echo '')}"
...
# Supervise ONE robot: restart on exit. Only repeated signal-style native crashes may
# disable A/V. Ordinary failures (DNS, login, temporary cloud/RTC errors) must keep A/V
# enabled so a recovered network restores the RTSP stream instead of leaving the robot
# permanently in control-only mode.
run_robot() {
    ...
    (
      ...
      if [ "$NR" -gt 1 ]; then          # distinct node/port/path only when there's more than one
        export EBO_NODE="ebo_${id}" EBO_RTSP_PATH="ebo_${id}" \
               EBO_RTSP_PORT="$((8554 + idx))" EBO_TALK_STREAM_PORT="$((8200 + idx))"
      fi
