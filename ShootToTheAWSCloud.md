开了branch 把项目移到AWS上去了，因此引发的一连串问题和对解决方案的探索。-补：后来又考虑了一下。因为现在加了 diagnostic agent，又跟云端那边做了一些联动，导致它跟当前上传的这个项目分歧有点太大了。我觉得在当前项目里开 branch 可能不是一个很好的选择。所以后续等我调得差不多、要发上来的时候，我会建一个新的 repository，而不是在这里面开 branch。
