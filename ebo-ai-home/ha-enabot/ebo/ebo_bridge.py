...
import ebo_cloud
from pcm_talk import PcmTalkServer
...
def _env_bool(name, default=False):
    """Read a strict but user-friendly boolean environment variable."""
    ...
    if value in ("0", "false", "no", "off"):
        return False
    raise ValueError(
        "%s must be true/false, 1/0, yes/no, or on/off (got %r)" % (name, raw_value)
    )


def _build_apm_config(aec_enabled, noise_suppression_enabled, agc_enabled):
    """Build an explicit Agora APM configuration with no hidden processors enabled."""
    from agora.rtc.agora_base import (
        APMConfig, AgcConfig, AiAecConfig, AiNsConfig, BghvsCConfig,
    )

    return APMConfig(
        ai_aec_config=AiAecConfig(enabled=aec_enabled),
        ai_ns_config=AiNsConfig(
            ns_enabled=noise_suppression_enabled,
            ai_ns_enabled=noise_suppression_enabled,
        ),
        # Background-human-voice suppression is a separate algorithm. Keep it off unless it
        # becomes an intentional, independently tested feature.
        bghvs_c_config=BghvsCConfig(enabled=False),
        agc_config=AgcConfig(enabled=agc_enabled),
        enable_dump=False,
    )
...
class Bridge:
    def __init__(self, session, mqtt_conf, provider=None, robot_id=None):
        ...
        self.listen_on = self._ui.get("listen", True) is not False
        self.audio_health = AudioHealth()
        self._last_audio_health_status = None
        ...
        self.agora_aec_enabled = _env_bool("EBO_AGORA_AEC_ENABLED", False)
        self.agora_noise_suppression_enabled = _env_bool(
            "EBO_AGORA_NOISE_SUPPRESSION_ENABLED", False
        )
        self.agora_agc_enabled = _env_bool("EBO_AGORA_AGC_ENABLED", False)
        self.agora_apm_enabled = any((
            self.agora_aec_enabled,
            self.agora_noise_suppression_enabled,
            self.agora_agc_enabled,
        ))
        self._apm_filter_rc = None
        self._talk_lock = threading.Lock()
        self._talk_wake_lock = threading.Lock()
        self._talk_wake_running = False
        ...
        self._pcm_stream = None         # active low-latency PCM stream, if any
        self._pcm_server = None
        ...
        self._rejoin_lock = threading.Lock()  # only one cloud/Agora rebuild may run at a time
    ...
    def connect_agora(self):
        ...
        class RtcObs(IRTCConnectionObserver):
            def on_connected(o, conn, info, reason):
                ...
                self._session_started_at = time.time()
                log("[RTC] connected")
                # A talk request may have arrived while this RTC session was waking.
                # Resume it only after the new connection has an audio sender.
                threading.Thread(target=self._resume_queued_talk, daemon=True).start()
        ...
        if self.video_enabled:
            try:
                ...
                pass
        if self.agora_apm_enabled:
            scfg.enable_apm = True
            scfg.apm_config = _build_apm_config(
                self.agora_aec_enabled,
                self.agora_noise_suppression_enabled,
                self.agora_agc_enabled,
            )
        log("[audio-apm] requested: aec=%s noise_suppression=%s agc=%s apm=%s" % (
            "on" if self.agora_aec_enabled else "off",
            "on" if self.agora_noise_suppression_enabled else "off",
            "on" if self.agora_agc_enabled else "off",
            "on" if self.agora_apm_enabled else "off",
        ))
        init_rc = svc.initialize(scfg)
        if init_rc != 0:
            raise RuntimeError(
                "Agora service initialization failed (rc=%s, APM requested=%s)" % (
                    init_rc, self.agora_apm_enabled
                )
            )
        log("[audio-apm] Agora service initialized (rc=0, apm=%s)" % (
            "on" if self.agora_apm_enabled else "off"
        ))
        self.svc = svc
        ...
        if self.audio_enabled:
            try:
                ...
                params = [
                    '{"che.audio.codec_unfallback":[0,8,9]}',
                    '{"che.audio.aec.enable":%s}' % (
                        "true" if self.agora_aec_enabled else "false"
                    ),
                ]
                if pt_opt not in ("auto", ""):
                    ...
                    pt = "auto"
                for kv in params:
                    rc = gp.set_parameters(kv)
                    if rc != 0:
                        log("[audio] global parameter rejected rc=%s: %s" % (rc, kv))
                log("[audio] codec params set on ENGINE before join "
                    "(codec_unfallback [0,8,9], payload_type %s, classic_aec %s)" % (
                        pt, "on" if self.agora_aec_enabled else "off"
                    ))
            except Exception as e:
        ...
        self._observers_registered = False
        if self.agora_apm_enabled:
            self._install_apm_filter_result_logger()
        ...
        if self.video_enabled:
            ...
            # MQTT connected before Agora during startup, so its first retained camera publish can
            # race ahead of the panel subscriber. Re-announce after the media pipeline is actually
            # ready; this also keeps HA's camera switch honest after every reconnect.
    ...
    def _install_apm_filter_result_logger(self):
        """Expose the SDK's otherwise-suppressed remote-track APM result in our clean log.

        Agora calls this private connection method from its own local-user subscribed callback.
        ebo_log intentionally discards third-party stdout, including Agora's print of this return
        code, so wrap the method without changing the SDK's behavior.
        """
        try:
            original = self.rtc._set_apm_filter_properties

            def _logged(remote_audio_track_handle, user_id_str):
                rc = original(remote_audio_track_handle, user_id_str)
                self._apm_filter_rc = rc
                log("[audio-apm] remote playback filter attached uid=%s rc=%s" % (
                    user_id_str, rc
                ))
                return rc

            self.rtc._set_apm_filter_properties = _logged
        except Exception as e:
            log("[audio-apm] could not install filter result logger:", e, level="error")
    ...
    def _register_audio_diag(self):
        ...
        class LUObs(IRTCLocalUserObserver):
            ...
            def on_remote_audio_track_statistics(o, lu, track, stats):
                audio_health.statistics(
                    int(getattr(stats, "received_bytes", 0)),
                    int(getattr(stats, "received_bitrate", 0)),
                )
                o._stat_n[0] += 1
        ...
        try:
            ...
            def _audio_watchdog(obs=self._audio_obs):
                ...
                while time.time() < end:
                    ...
                    time.sleep(0.5)
                if (obs._n[0] == 0 and self.listen_on
                        and obs is self._audio_obs):
    ...
    def _talk_ready(self):
        """Whether the current RTC session can publish PCM to the robot."""
        return (self.connected and self.rtc_state == "connected" and self.rtc
                and getattr(self.rtc, "_audio_sender", None) is not None)

    def _resume_queued_talk(self):
        """Wait for a waking/rejoining RTC session, then start queued audio."""
        for _ in range(120):  # deep sleep/cloud refresh can take tens of seconds
            with self._talk_lock:
            ...
            if self._talk_ready():
                ...
                return
            time.sleep(0.25)
        log("[talk] queued audio is still waiting: RTC did not become ready")

    def _wake_for_talk(self):
        """Wake/rejoin once for all audio queued during standby."""
        try:
            self._wake_full()
            self._resume_queued_talk()
        finally:
            with self._talk_wake_lock:
                self._talk_wake_running = False
    ...
    def _audio_tx_loop(self, sender):
        ...
        while self._tx_run:
            with self._talk_lock:
                pcm_stream = self._pcm_stream
            if pcm_stream:
                if pcm_stream.stopped.is_set():
                    with self._talk_lock:
                        if self._pcm_stream is pcm_stream:
                            self._pcm_stream = None
                    continue
                chunk = pcm_stream.pop_frame()
                if chunk:
                    try:
                        sender.send_audio_pcm_data(self._mk_pcm(chunk))
                        pcm_stream.mark_played()
                    except Exception as e:
                        log("[talk-stream] send error:", e)
                        pcm_stream.stop("agora_send_failed")
                    time.sleep(0.02)
                    continue
                if pcm_stream.drained():
                    pcm_stream.complete()
                    with self._talk_lock:
                        if self._pcm_stream is pcm_stream:
                            self._pcm_stream = None
                    log("[talk-stream] done — %d ms played" % pcm_stream.played_ms)
                    continue
                # Keep the Agora publisher clocked while waiting for another PCM delta.
                try:
    ...
    def _activate_pcm_stream(self, stream):
        """Preempt URL playback and make a WebSocket stream the active talk source."""
        if not (self.audio_enabled or self.talk_enabled):
            raise RuntimeError("talk is disabled")
        if not self._talk_ready():
            raise RuntimeError("robot RTC audio sender is not ready")
        self._talk_stop = True
        with self._talk_lock:
            previous = self._pcm_stream
            self._tx_queue = []
            self._pcm_stream = stream
        if previous and previous is not stream:
        ...
        if not self._tx_run:
            self._start_audio_tx()
        log("[talk-stream] ready:", stream.stream_id)

    def _release_pcm_stream(self, stream):
        with self._talk_lock:
            if self._pcm_stream is stream:
                self._pcm_stream = None
        log("[talk-stream] released %s at %d ms" % (stream.stream_id, stream.played_ms))

    def _start_pcm_server(self):
        port = int(os.environ.get("EBO_TALK_STREAM_PORT", "8200"))
        token = os.environ.get("EBO_API_TOKEN", "")
        if not token:
            log("[talk-stream] disabled: EBO_API_TOKEN is empty")
            return
    ...
    def _talk(self, source):
        ...
        if not self._talk_ready():
            log("[talk] queued while the robot wakes; playback will start when RTC is ready")
            with self._talk_wake_lock:
                if not self._talk_wake_running:
                    self._talk_wake_running = True
                    threading.Thread(target=self._wake_for_talk, daemon=True).start()
            return
    ...
    def _camera_feed(self, on):
        ...
        if on:
            ...
            # Reconnects reuse the same VideoPipeline. Keep exactly one diagnostic thread for it;
            # otherwise every reconnect leaves another permanent loop behind, all sending wake
            # commands at once when frames pause.
            if self._video_diag_thread is None or not self._video_diag_thread.is_alive():
                self._video_diag_thread = threading.Thread(target=self._video_diag, daemon=True)
                self._video_diag_thread.start()
    ...
    def _force_rejoin(self):
        ...
        now = time.time()
        # A robot/account hand-off can legitimately take tens of seconds (for example after the
        # phone app releases its live session). Rebuilding faster than this creates a feedback loop:
        # the first video frame arrives just as the next rebuild tears the session down.
        if now - getattr(self, "_last_rejoin", 0) < 90:
            return
        ...
        try:
            ...
            # A sender belongs to one RTC connection. Never let the old TX loop
            # survive into the replacement session; queued sources remain queued.
            self._stop_audio_tx()
            try:
                old_rtc, self.rtc = self.rtc, None
                if old_rtc:
                    try:
                        old_rtc.disconnect()
                    except Exception:
                    ...
                    try:
                        ...
                        pass
                old_svc, self.svc = self.svc, None
                if old_svc:
                    try:
                        old_svc.release()
                    except Exception:
                        pass
                # IMPORTANT: logout makes this RTM object unusable. The old implementation logged
                # out and then connect_agora() saw a non-None object and "reused" it, leaving the
                # robot present in RTC but unable to receive wake/camera/quality commands.
                old_rtm, self.rtm = self.rtm, None
                if old_rtm:
                    try:
                        old_rtm.logout()
                    except Exception:
    ...
    def _publish_discovery(self, c):
        ...
        for _t in ("laser/set", "speed/set", "move/+", "move/vector", "joystick", "sleep/set",
                   "wake", "say", "talk", "talk/stop", "listen/set", "microphone/set", "audio_tx/set", "volume/set", "talkback_volume/set",
    ...
    def _publish_audio_health(self):
        health = self.audio_health.snapshot(
            self.listen_on, self.rtc_state == "connected", self.audio_enabled
        )
        if self.mqtt:
            self.mqtt.publish("%s/audio_health" % NODE, json.dumps(health), retain=True)
        if health["status"] != self._last_audio_health_status:
            log("[audio-health] status=%s listen=%s bitrate=%s bytes=%s" % (
                health["status"], self.listen_on,
