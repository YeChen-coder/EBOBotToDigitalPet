...
- 听到孤立的“三十四”且没有正在接续的相关对话：不凭图片猜测含义，等待明确交流。
""".strip()
...
class RealtimeClient:
    ...
    def _on_message(self, _ws: websocket.WebSocketApp, raw: str) -> None:
        ...
        if kind == "session.updated":
            ...
            LOG.warning("input transcription failed: %s", error)
            ...
            LOG.error("Realtime server error: %s", error)
